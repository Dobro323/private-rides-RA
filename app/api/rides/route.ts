...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      ...
      ride_date, ride_time, passengers,
      payment_method, notes, lang = 'en',
    } = body
    ...
    await notifyNewRide({
      id: ride.id, client_name, client_email, client_phone,
      pickup_address, dropoff_address, ride_date, ride_time,
      passengers: parseInt(passengers) || 1,
      notes, payment_method, lang,
    })

    await emailBookingReceived({
      email: client_email, name: client_name,
      pickup: pickup_address, dropoff: dropoff_address,
      date: ride_date, time: ride_time, lang,
    })
